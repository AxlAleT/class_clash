...
class _QuizQuestionsScreenState extends ConsumerState<QuizQuestionsScreen> {
  ...
  bool get _hasUnsavedChanges {
    // Compare current state with initial state or check if there are unsaved questions
    final state = _controller.state;
    return state.isDirty || state.hasUnsavedQuestionChanges;
  }
        ...
        appBar: AppBar(
          ...
          leading: IconButton(
            ...
            onPressed: () async {
              ...
              // Determine where to navigate back to based on quiz state
              if (quizQuestionsState.quizModel.id != null && quizQuestionsState.quizModel.id!.isNotEmpty) {
                // If editing an existing quiz, go back to quiz list
                context.go('/quizzes');
              } else {
                // If creating a new quiz, go back to quiz data screen
